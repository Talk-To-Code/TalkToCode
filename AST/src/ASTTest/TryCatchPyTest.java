...
public class TryCatchPyTest extends TestCase {
	private static ASTParser parser;
	@Test
	public void testTryCatch() {
		try {
			InputStream in = new FileInputStream(new File("./input/TryCatchInputPy.txt"));
			InputStream out = new FileInputStream(new File("./output/TryCatchOutputPy.txt"));
			...
			while((temp=br.readLine())!=null){
				if(temp.length() >= 3 && temp.substring(0, 3).compareTo("try") == 0) {
